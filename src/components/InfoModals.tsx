...
export const InfoSection: React.FC<InfoSectionProps> = ({ section, onClose, currentUser }) => {
  ...
  return (
    <div className="fixed inset-0 glass-panel backdrop-blur-md flex items-center justify-center p-4 z-50 animate-fade-in text-left">
      <div className="max-w-3xl w-full max-h-[90vh] overflow-y-auto rounded-2xl border border-white/10 shadow-2xl relative">
        <GamerPanel
          variant="blue"
        ...
        >
          ...
          <button
            onClick={onClose}
            className="absolute top-4 right-4 p-1 rounded glass-panel hover:glass-panel text-gray-400 hover:text-gray-200 transition-colors cursor-pointer"
          >
            <X className="w-5 h-5" />
          </button>
          ...
          <div className="mt-2 space-y-6">
            ...
            {section === 'about' && (
              <div className="space-y-4 text-gray-300 text-sm leading-relaxed">
                ...
                <p>
                  O <strong className="text-cyan-400">X1 ALZ</strong> foi idealizado para ser a maior e mais respeitada plataforma de facilitação de apostas PvP (Gladiador contra Gladiador) para jogadores do renomado MMORPG <strong className="text-gray-100">Cabal Neo</strong>.
                </p>
                <p>
            ...
            {section === 'how_to' && (
              <div className="space-y-4 text-gray-300 text-sm">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="p-4 glass-panel border border-white/10 rounded-xl space-y-2">
                    ...
                    <h4 className="font-display font-bold text-gray-200 text-xs uppercase">REGISTRO DA SALA</h4>
                    <p className="text-gray-400 text-xxs leading-normal">Defina a aposta (ex: 1b ALZ), gere o código e compartilhe com seu adversário do Cabal.</p>
                  </div>
                  ...
                  <div className="p-4 glass-panel border border-white/10 rounded-xl space-y-2">
                    ...
                    <h4 className="font-display font-bold text-gray-200 text-xs uppercase">DEPÓSITO SEGURO</h4>
                    <p className="text-gray-400 text-xxs leading-normal">Ambos os jogadores enviam os ALZ para o personagem do admin <strong className="text-amber-400 font-mono">X1Alz</strong> in-game.</p>
                  </div>
                  ...
                  <div className="p-4 glass-panel border border-white/10 rounded-xl space-y-2">
                    ...
                    <h4 className="font-display font-bold text-gray-200 text-xs uppercase">SORTEIO & RESGATE</h4>
                    <p className="text-gray-400 text-xxs leading-normal">O sorteio é feito ao vivo na roleta de probabilidade e o admin faz o trade dos ALZ para o vencedor.</p>
                  </div>
                </div>

                <div className="glass-panel border border-white/10 rounded-xl p-4 space-y-1.5 mt-2">
            ...
            {/* REGRAS (RULES) */}
            {section === 'rules' && (
              <div className="space-y-4 text-gray-300 text-xs leading-relaxed font-sans">
                ...
                <div className="space-y-3.5 divide-y divide-neutral-900">
                  <div className="pt-2">
                    <span className="font-bold text-gray-200 block mb-1">1. ENVIO DO ALZ PARA O PERSONAGEM CORRETO</span>
                  ...
                  <div className="pt-3">
                    <span className="font-bold text-gray-200 block mb-1">2. CANCELAMENTOS DE SALAS</span>
                  ...
                  <div className="pt-3">
                    <span className="font-bold text-gray-200 block mb-1">3. RESULTADO SOBERANO</span>
                  ...
                  <div className="pt-3">
                    <span className="font-bold text-gray-200 block mb-1">4. PRAZO DE PAGAMENTO DO PRÊMIO</span>
                    <span>O administrador se compromete a realizar o trade de pagamento de ALZ para o vencedor dentro do jogo em no máximo <strong className="text-gray-100">12 horas</strong> após o término do sorteio, respeitando o horário de funcionamento das 12:00 às 18:00.</span>
                  </div>
                </div>
              </div>
            )}
            ...
            {section === 'ranking' && (
              <div className="space-y-4">
                {rankings.length === 0 ? (
                  <p className="text-gray-500 text-xs py-4 text-center">Nenhum guerreiro registrado no ranking ainda.</p>
                ) : (
                    ...
                    <table className="w-full text-left border-collapse text-xs">
                      ...
                      <tbody className="divide-y divide-neutral-900">
                        {rankings.map((r, idx) => {
                          ...
                          return (
                            <tr key={r.userId} className="hover:glass-panel">
                              <td className="py-3 font-mono font-bold text-center text-gray-300">{medal}</td>
                              <td className="py-3">
                                <span className="font-mono font-bold text-gray-200 uppercase">{r.gameNick}</span>
                                <span className="text-[10px] text-gray-500 block">Real: {r.name}</span>
                              </td>
                              <td className="py-3 font-mono text-center text-gray-400">
                                <span className="text-emerald-400">{r.wins}</span> / <span className="text-red-400">{r.losses}</span>
                              </td>
                              <td className="py-3 font-mono text-right text-gray-400">{formatALZ(r.totalBet)}</td>
            ...
            {section === 'history' && (
              <div className="space-y-4">
                {allFinishedRooms.length === 0 ? (
                  <p className="text-gray-500 text-xs py-4 text-center">Nenhum combate concluído registrado no histórico global ainda.</p>
                ) : (
                  <div className="overflow-x-auto glass-panel rounded-xl border border-white/10 p-4">
                    <table className="w-full text-left border-collapse text-xs">
                      <thead>
                      ...
                      <tbody className="divide-y divide-neutral-900">
                        {allFinishedRooms.map((room) => (
                          <tr key={room.id} className="hover:glass-panel font-sans">
                            <td className="py-3 font-mono font-bold text-gray-300 uppercase">{room.code}</td>
                            <td className="py-3 font-mono text-gray-400">{room.player1Nick}</td>
                            <td className="py-3 font-mono text-gray-400">{room.player2Nick || 'Bot'}</td>
                            <td className="py-3 font-mono text-gray-400">{formatALZ(room.betAmount)}</td>
