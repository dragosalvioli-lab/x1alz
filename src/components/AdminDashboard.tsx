  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-8 animate-fade-in text-left">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-gray-500 font-mono">ID: {adminUser.gameNick}</span>
          </div>
          <h1 className="font-display font-black text-3xl uppercase tracking-wider text-gray-100 mt-1">
              ...
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2.5 rounded-lg border font-display text-xs font-bold uppercase tracking-wider transition-all cursor-pointer ${
                  activeTab === tab.id
                    ...
                    : 'bg-white/5 border-white/5 text-gray-400 hover:glass-panel hover:text-gray-200'
                }`}
              >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="p-5 bg-white/5 border border-white/5 rounded-xl relative overflow-hidden shadow-md">
          <div className="absolute top-0 right-0 p-3 opacity-10">
            <Coins className="w-14 h-14 text-amber-400" />
          </div>
          <span className="text-xxs font-mono text-gray-500 uppercase tracking-wider block">Acumulado Recebido</span>
          <span className="text-xl font-mono font-bold text-gray-200 block mt-1">{formatALZ(metrics.totalReceived)}</span>
        ...
        <div className="p-5 bg-white/5 border border-white/5 rounded-xl relative overflow-hidden shadow-md">
          ...
          <span className="text-xxs font-mono text-gray-500 uppercase tracking-wider block">Total Pago aos Vencedores</span>
          <span className="text-xl font-mono font-bold text-emerald-400 block mt-1">{formatALZ(metrics.totalPaid)}</span>
          <span className="text-xxs text-gray-400 block mt-1">Quitados in-game</span>
        </div>
        ...
        <div className="p-5 bg-white/5 border border-white/5 rounded-xl relative overflow-hidden shadow-md">
          ...
          <span className="text-xxs font-mono text-gray-500 uppercase tracking-wider block">Lucro Casa (Taxa {db.getSettings().houseFeePercent}%)</span>
          <span className="text-xl font-mono font-bold text-amber-400 block mt-1">{formatALZ(metrics.totalProfit)}</span>
          <span className="text-xxs text-gray-400 block mt-1">Hoje: <strong className="text-amber-400 font-mono">{formatALZ(metrics.profitToday)}</strong></span>
        </div>
        ...
        <div className="p-5 bg-white/5 border border-white/5 rounded-xl relative overflow-hidden shadow-md">
          ...
          <span className="text-xxs font-mono text-gray-500 uppercase tracking-wider block">Jogadores Online</span>
          <span className="text-xl font-mono font-bold text-cyan-400 block mt-1">{metrics.onlinePlayers} ativos</span>
          <span className="text-xxs text-gray-400 block mt-1">Salas abertas: {metrics.activeRooms}</span>
        </div>
      ...
      <div className="space-y-6">
        ...
        {activeTab === 'rooms' && (
          <GamerPanel variant="gold" title="Lista Geral de Salas de Aposta" subtitle="Confirme recebimentos, cancele duelos ou force sorteios para teste">
            ...
            {/* Filter bar */}
            <div className="flex flex-col sm:flex-row gap-3 mb-6 glass-panel p-4 border border-white/5 rounded-xl">
              <div className="flex-1">
                <label className="block text-xxs font-mono text-gray-500 uppercase mb-1">Buscar por Código</label>
                <input
                  type="text"
                  value={roomFilterCode}
                  onChange={(e) => setRoomFilterCode(e.target.value)}
                  placeholder="Ex: X1-1250"
                  className="w-full bg-white/5 border border-white/5 rounded-lg py-2 px-3 text-xs text-gray-200 placeholder-neutral-600 focus:outline-none focus:border-amber-500 uppercase font-mono"
                />
              </div>

              <div className="w-full sm:w-64">
                <label className="block text-xxs font-mono text-gray-500 uppercase mb-1">Filtrar por Status</label>
                <select
                  value={roomFilterStatus}
                  onChange={(e) => setRoomFilterStatus(e.target.value)}
                  className="w-full bg-white/5 border border-white/5 rounded-lg py-2 px-3 text-xs text-gray-200 focus:outline-none focus:border-amber-500 cursor-pointer"
                >
            ...
            {filteredRooms.length === 0 ? (
              <p className="text-gray-500 text-xs py-8 text-center font-sans">Nenhuma sala encontrada correspondente aos filtros ativos.</p>
            ) : (
              <div className="space-y-3.5">
                {filteredRooms.map((room) => (
                  <div
                    key={room.id}
                    className="p-4 glass-panel border border-white/5/80 rounded-xl hover:border-neutral-700/80 transition-all flex flex-col md:flex-row md:items-center justify-between gap-4"
                  >
                    {/* Left: Info details */}
                    <div className="text-left space-y-1">
                      ...
                      <div className="text-xxs text-gray-400 font-sans space-y-0.5">
                        <p>Jogador 1: <strong className="text-gray-200 font-mono">{room.player1Nick}</strong> ({room.player1Name})</p>
                        <p>Jogador 2: <strong className="text-gray-200 font-mono">{room.player2Nick || 'Aguardando...'}</strong> {room.player2Name ? `(${room.player2Name})` : ''}</p>
                        <p className="text-[10px] text-gray-500">Criada em: {new Date(room.createdAt).toLocaleString()}</p>
                      </div>
                    ...
                    <div className="flex flex-wrap gap-2 justify-start md:justify-end items-center shrink-0">
                      ...
                      {room.status === 'finished' && (
                        <div className="text-right text-xs font-mono">
                          <p className="text-gray-500">Taxa da Casa: <strong className="text-amber-400 font-bold">+{formatALZ(room.houseFeeAmount)}</strong></p>
                          <p className="text-gray-400">Pago ao Vencedor: {formatALZ(room.netWinnerAmount)}</p>
                        </div>
                      )}

                      {room.status === 'cancelled' && (
                        <span className="text-xxs text-gray-500 font-mono italic">Reembolsado aos jogadores</span>
                      )}
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse text-xs font-sans">
                  ...
                  <tbody className="divide-y divide-neutral-900/50">
                    {payments.map((p) => (
                      <tr key={p.id} className="hover:bg-white/5/30">
                        <td className="py-3.5">
                          <div className="font-mono font-bold text-gray-300 uppercase">{p.winnerNick}</div>
                          <div className="text-[10px] text-gray-500">Real: {p.winnerName}</div>
                        </td>
                        <td className="py-3.5 font-mono text-gray-400">{formatALZ(p.betAmount)}</td>
                        <td className="py-3.5 font-mono text-gray-400">{formatALZ(p.grossTotal)}</td>
                        ...
                        <td className="py-3.5 text-right shrink-0">
                          {p.status === 'pending' ? (
                            ...
                            <span className="text-xxs text-gray-500 font-mono italic">
        ...
        {activeTab === 'tickets' && (
          <GamerPanel title="Gerenciamento de Suporte" icon={<HeartHandshake className="w-5 h-5" />} neonColor="amber">
            <div className="flex gap-4 h-[500px]">
              {/* Ticket List */}
              <div className="w-1/3 flex flex-col border border-white/5 rounded-xl overflow-hidden glass-panel">
                <div className="p-3 bg-white/5 border-b border-white/5">
                  <h4 className="text-xs font-bold font-display text-gray-300 uppercase">Tickets Abertos ({tickets.filter(t => t.status === 'open').length})</h4>
                </div>
                <div className="flex-1 overflow-y-auto p-2 space-y-2 scrollbar-thin scrollbar-thumb-neutral-800">
                  {tickets.map(t => (
                    <button
                      ...
                      className={`w-full text-left p-3 rounded-lg border transition-colors cursor-pointer ${selectedTicketId === t.id ? 'bg-amber-950/40 border-amber-500/50' : 'bg-transparent border-white/5 hover:border-neutral-600'}`}
                    >
                      <div className="flex justify-between items-start mb-1">
                        <span className="text-xs font-bold text-gray-200">{t.userNick}</span>
                      ...
                      <div className="text-xxs text-gray-500 mt-1">{new Date(t.updatedAt).toLocaleString()}</div>
                    </button>
                  ))}
                  {tickets.length === 0 && <div className="text-center p-4 text-xs text-gray-500 font-mono">Nenhum ticket encontrado.</div>}
                </div>
              </div>
              
              {/* Ticket Details */}
              <div className="w-2/3 flex flex-col border border-white/5 rounded-xl overflow-hidden glass-panel">
                {selectedTicketId ? (
                  (() => {
                    ...
                    return (
                      <>
                        <div className="p-3 bg-white/5 border-b border-white/5 flex justify-between items-center">
                          <div>
                            <h4 className="text-sm font-bold font-display text-gray-200">{ticket.userNick} - <span className="text-amber-400">{ticket.category}</span></h4>
                          ...
                          <div className="flex gap-2">
                            {ticket.status !== 'closed' && (
                              <button 
                                ...
                                className="px-3 py-1 glass-panel hover:glass-panel text-gray-300 text-xs font-bold rounded cursor-pointer"
                              >
                                Fechar Ticket
                              </button>
                            )}
                        ...
                        <div className="flex-1 p-4 overflow-y-auto space-y-4 scrollbar-thin scrollbar-thumb-neutral-800">
                          {ticket.messages.map(m => (
                            <div key={m.id} className={`flex flex-col ${m.senderRole === 'admin' ? 'items-end' : 'items-start'}`}>
                              ...
                              <div className={`p-3 rounded-lg text-sm max-w-[80%] ${m.senderRole === 'admin' ? 'bg-amber-950/40 border border-amber-900/50 text-gray-200' : 'bg-transparent border border-white/5 text-gray-300'}`}>
                                {m.text}
                              </div>
                            </div>
                          ))}
                        ...
                        {ticket.status !== 'closed' && (
                          <form 
                            ...
                            className="p-3 bg-transparent border-t border-white/5 flex gap-2"
                          >
                            <input
                              type="text"
                              ...
                              className="flex-1 bg-white/5 border border-white/5 rounded px-3 py-2 text-sm text-gray-200 focus:outline-none focus:border-amber-500"
                            />
        ...
        {activeTab === 'reports' && (
          <div className="space-y-6">
            <GamerPanel variant="gold" title="Relatório de Auditoria e Fechamento" subtitle="Exportações detalhadas para declaração de caixa">
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 glass-panel p-6 border border-white/5 rounded-xl">
                <div>
                  <h4 className="font-display font-bold text-gray-200 text-sm uppercase">Exportar Planilha Completa</h4>
                  <p className="text-gray-400 text-xs mt-1">Gera um arquivo de auditoria (.CSV) contendo todas as apostas liquidadas, taxas recolhidas e contas vinculadas.</p>
                </div>
                <GamerButton
                  onClick={exportToCSV}
                  variant="gold"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                <div>
                  ...
                  <div className="glass-panel border border-white/5 rounded-xl p-5 space-y-3 font-mono text-xs text-gray-400">
                    <div className="flex justify-between">
                      <span>Total Recebido Hoje:</span>
                      <span className="text-gray-200 font-bold">{formatALZ(metrics.receivedToday)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Total Pago Hoje aos Jogadores:</span>
                      <span className="text-emerald-400 font-bold">{formatALZ(metrics.paidToday)}</span>
                    </div>
                    <div className="h-[1px] glass-panel" />
                    <div className="flex justify-between">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="text-gray-200">{formatALZ(metrics.totalProfit * 1.4)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Lucro Estimado Mensal:</span>
                      <span className="text-gray-200">{formatALZ(metrics.totalProfit * 3.2)}</span>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="glass-panel border border-white/5 rounded-xl p-4 h-40 overflow-y-auto font-mono text-[10px] text-gray-400 space-y-2">
                    {logs.map((log) => (
                      <div key={log.id} className="border-b border-neutral-900 pb-1.5 last:border-0">
                        <span className="text-gray-500">[{formatBrasiliaTimeOnly(log.createdAt)}]</span>{' '}
          ...
          <GamerPanel variant="gold" title="Parâmetros Globais do Sistema" subtitle="Altere horários de fechamento, taxas administrativas e credenciais in-game">
            ...
            <form onSubmit={handleUpdateSettings} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  <label className="block text-xxs font-mono text-gray-400 uppercase tracking-wider mb-2">Horário de Funcionamento (Abertura e Fechamento)</label>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <span className="text-[10px] text-gray-500 font-mono block mb-1">Hora de Abertura</span>
                      <input
                        type="text"
                        value={settings.openingHour}
                        onChange={(e) => setSettings({ ...settings, openingHour: e.target.value })}
                        placeholder="12:00"
                        className="w-full glass-panel border border-white/5 rounded-lg py-2.5 px-3 text-xs font-mono text-gray-200 focus:outline-none focus:border-amber-500"
                      />
                    </div>
                    <div>
                      <span className="text-[10px] text-gray-500 font-mono block mb-1">Hora de Fechamento</span>
                      <input
                        type="text"
                  ...
                  <span className="text-[10px] text-gray-500 mt-1.5 block font-sans">Fora desse horário, o painel restringe apostas e ativa a tela de fechamento.</span>
                </div>

                <div>
                  <label className="block text-xxs font-mono text-gray-400 uppercase tracking-wider mb-2">Porcentagem de Taxa Retida pela Casa</label>
                  <div>
                    <span className="text-[10px] text-gray-500 font-mono block mb-1">Taxa (%)</span>
                    <input
                      type="number"
                  ...
                  <span className="text-[10px] text-gray-500 mt-1.5 block font-sans">Retida no encerramento de cada duelo para cobrir custos de manutenção.</span>
                </div>

                <div>
                  <label className="block text-xxs font-mono text-gray-400 uppercase tracking-wider mb-2">Nome do Personagem Receptor (Cabal Neo)</label>
                  <div>
                    <span className="text-[10px] text-gray-500 font-mono block mb-1">Nome do Char</span>
                    <input
                      type="text"
                  ...
                  <span className="text-[10px] text-gray-500 mt-1.5 block font-sans">Nome exibido para os jogadores enviarem os ALZ apostados in-game.</span>
                </div>

                <div>
                  <label className="block text-xxs font-mono text-gray-400 uppercase tracking-wider mb-2">Mensagem de Anúncio Inicial</label>
                  <div>
                    <span className="text-[10px] text-gray-500 font-mono block mb-1">Texto de Boas-Vindas</span>
                    <input
                      type="text"
                      value={settings.welcomeMessage}
                      onChange={(e) => setSettings({ ...settings, welcomeMessage: e.target.value })}
                      placeholder="Bem-vindo ao X1 ALZ!"
                      className="w-full glass-panel border border-white/5 rounded-lg py-2.5 px-3 text-xs text-gray-200 focus:outline-none focus:border-amber-500"
                    />
                  </div>
                  <span className="text-[10px] text-gray-500 mt-1.5 block font-sans">Frase exibida no cabeçalho do portal para todos os usuários logados.</span>
                </div>
